    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(
                f"{BASE_URL}/api/fraud/", headers=HEADERS, json=message
            )
            ...
            if response.status_code == 200:
                ...
                if response.headers.get("content-type") == "audio/mpeg":
                    response_text = response.headers.get(
                        "X-Response-Text", "No text in headers"
                    )
                    message_id = response.headers.get("X-Message-Id", "No message ID")
                    conversation_id = response.headers.get(
                        "X-Conversation-Id", "No conversation ID"
                    )
                    service_type = response.headers.get(
                        "X-Service-Type", "No service type"
                    )
                    target_name = response.headers.get(
                        "X-Target-Name", "No target name"
                    )
                    target_phone = response.headers.get(
                        "X-Target-Phone", "No target phone"
                    )
                    ...
                    print(
                        f"Target Phone: {data.get('target_phone', 'No target phone')}"
                    )
                    if "error" in data:
